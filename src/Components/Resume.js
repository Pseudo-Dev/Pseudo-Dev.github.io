  ...
  render() {
    ...
    if(this.props.data){
      ...
      var work = this.props.data.work.map(function(work){
        var projectImage = 'images/portfolio/' + work.image;
        return <div key={work.company}><h3>{work.company}</h3>
            <p className="info">{work.title}<span>&bull;</span> <em className="date">{work.years}</em></p>
            <p>{work.description}</p>
            <div key={work.workTitle} className="columns portfolio-item">
            <div style={{position: "relative", left: "-3.3%"}} className="item-wrap">
              <a href={work.url} title={work.workTitle}>
                <img alt={work.workTitle} src={projectImage} />
